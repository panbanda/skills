...
export const PandaPlugin = async ({ client, directory }) => {
  const homeDir = os.homedir();
  const projectSkillsDir = path.join(directory, '.opencode/skills');
  // Derive panda skills dir from plugin location (works for both symlinked and local installs)
  const pandaSkillsDir = path.resolve(__dirname, '../../skills');
  const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

  // Helper to generate bootstrap content
  const getBootstrapContent = (compact = false) => {
    const usingPandaPath = skillsCore.resolveSkillPath('using-panda', pandaSkillsDir, personalSkillsDir);
    if (!usingPandaPath) return null;

    const fullContent = fs.readFileSync(usingPandaPath.skillFile, 'utf8');
...
**Skills naming (priority order):** project: > personal > panda:`
...
- Panda skills: \`panda:skill-name\`
- Project skills override personal, which override panda when names match`;

    return `<EXTREMELY_IMPORTANT>
You have panda.

**IMPORTANT: The using-panda skill content is included below. It is ALREADY LOADED - you are currently following it. Do NOT use the use_skill tool to load "using-panda" - that would be redundant. Use use_skill only for OTHER skills.**
  ...
  return {
    tool: {
      use_skill: tool({
        ...
        args: {
          skill_name: tool.schema.string().describe('Name of the skill to load (e.g., "panda:brainstorming", "my-custom-skill", or "project:my-skill")')
        },
        execute: async (args, context) => {
          const { skill_name } = args;

          // Resolve with priority: project > personal > panda
          ...
          if (forceProject || !skill_name.startsWith('panda:')) {
          ...
          // Fall back to personal/panda resolution
          if (!resolved && !forceProject) {
            resolved = skillsCore.resolveSkillPath(skill_name, pandaSkillsDir, personalSkillsDir);
          }
...
# ============================================`;
      ...
      find_skills: tool({
        description: 'List all available skills in the project, personal, and panda skill libraries.',
        args: {},
        execute: async (args, context) => {
          ...
          const pandaSkills = skillsCore.findSkillsInDir(pandaSkillsDir, 'panda', 3);

          // Priority: project > personal > panda
          const allSkills = [...projectSkills, ...personalSkills, ...pandaSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install panda skills to ~/.config/opencode/panda/skills/ or add project skills to .opencode/skills/';
          }
