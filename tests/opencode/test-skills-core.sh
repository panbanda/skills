...
# Create skills in personal and panda locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/panda-skills/shared-skill"
mkdir -p "$TEST_HOME/panda-skills/unique-skill"
...
cat > "$TEST_HOME/panda-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Panda version
---
# Panda Shared
EOF

cat > "$TEST_HOME/panda-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in panda
---
# Unique
EOF
...
function resolveSkillPath(skillName, pandaDir, personalDir) {
    const forcePanda = skillName.startsWith('panda:');
    const actualSkillName = forcePanda ? skillName.replace(/^panda:/, '') : skillName;

    if (!forcePanda && personalDir) {
    ...
    if (pandaDir) {
        const pandaPath = path.join(pandaDir, actualSkillName);
        const pandaSkillFile = path.join(pandaPath, 'SKILL.md');
        if (fs.existsSync(pandaSkillFile)) {
            return {
                skillFile: pandaSkillFile,
                sourceType: 'panda',
                skillPath: actualSkillName
            };
        }
...
const pandaDir = '$TEST_HOME/panda-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', pandaDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: panda: prefix should force panda
const forced = resolveSkillPath('panda:shared-skill', pandaDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to panda
const unique = resolveSkillPath('unique-skill', pandaDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', pandaDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow panda skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"panda"'; then
    echo "  [PASS] panda: prefix forces panda resolution"
else
    echo "  [FAIL] panda: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"panda"'; then
    echo "  [PASS] Unique panda skills are found"
else
    echo "  [FAIL] Unique panda skills not found"
    exit 1
fi
