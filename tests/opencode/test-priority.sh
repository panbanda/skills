...
# Verifies that skills are resolved with correct priority: project > personal > panda
...
# 1. Create in panda location (lowest priority)
mkdir -p "$HOME/.config/opencode/panda/skills/priority-test"
cat > "$HOME/.config/opencode/panda/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Panda version of priority test skill
---
# Priority Test Skill (Panda Version)

This is the PANDA version of the priority test skill.

PRIORITY_MARKER_PANDA_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/panda/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Panda version exists"
else
    echo "  [FAIL] Panda version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides panda
echo ""
echo "Test 2: Testing personal > panda priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides panda)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PANDA_VERSION"; then
    echo "  [FAIL] Panda version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|panda" | head -10
fi

# Test 3: Test that project overrides both personal and panda
echo ""
echo "Test 3: Testing project > personal > panda priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Panda version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit panda: prefix bypasses priority
echo ""
echo "Test 4: Testing panda: prefix forces panda version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load panda:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PANDA_VERSION"; then
    echo "  [PASS] panda: prefix correctly forces panda version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] panda: prefix did not force panda version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
