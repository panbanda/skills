...
Complete guide for using Panda with [OpenCode.ai](https://opencode.ai).
...
```
Clone https://github.com/obra/panda to ~/.config/opencode/panda, then create directory ~/.config/opencode/plugin, then symlink ~/.config/opencode/panda/.opencode/plugin/panda.js to ~/.config/opencode/plugin/panda.js, then restart opencode.
```
...
ln -sf ~/.config/opencode/panda/.opencode/plugin/panda.js .opencode/plugin/panda.js
```

#### 3. Restart OpenCode

Restart OpenCode to load the plugin. Panda will automatically activate.
...
3. **Panda skills** (`~/.config/opencode/panda/skills/`)

You can force resolution to a specific level:
- `project:skill-name` - Force project skill
- `skill-name` - Search project → personal → panda
- `panda:skill-name` - Force panda skill

## Features

### Automatic Context Injection

The plugin automatically injects panda context via the chat.message hook on every session. No manual configuration needed.
...
The plugin listens for `session.compacted` events and automatically re-injects the core panda bootstrap to maintain functionality after context compaction.
...
**Location:** `~/.config/opencode/panda/.opencode/plugin/panda.js`
...
**Location:** `~/.config/opencode/panda/lib/skills-core.js`
...
2. Check symlink: `ls -l ~/.config/opencode/plugin/panda.js`
3. Check OpenCode logs: `opencode run "test" --print-logs --log-level DEBUG`
4. Look for: `service=plugin path=file:///.../panda.js loading plugin`

### Skills not found

1. Verify skills directory: `ls ~/.config/opencode/panda/skills`
...
3. Test plugin manually: `node --input-type=module -e "import('file://~/.config/opencode/plugin/panda.js').then(m => console.log(Object.keys(m)))"`

### Context not injecting

1. Check if chat.message hook is working
2. Verify using-panda skill exists
3. Check OpenCode version (requires recent version with plugin support)

## Getting Help

- Report issues: https://github.com/obra/panda/issues
- Main documentation: https://github.com/obra/panda
